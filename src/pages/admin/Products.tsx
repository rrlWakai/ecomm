...
export function AdminProductsPage() {
  ...
  const bulkRestock = async () => {
    if (!selected.length) return;
    for (const id of selected) await supabase.from('products').update({ stock: 25 }).eq('id', id);
    setSelected([]);
    await products.refetch();
  };
  ...
  return (
    <main className="mx-auto max-w-6xl p-4 space-y-4">
      ...
      <form className="bg-white border rounded p-3 grid md:grid-cols-3 gap-2" onSubmit={createProduct}>
        ...
        <select name="category_id" className="border rounded p-2"><option value="">No category</option>{(categories.data ?? []).map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
