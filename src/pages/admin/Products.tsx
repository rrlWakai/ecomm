...
export function AdminProductsPage() {
  ...
  const lowStock = useMemo(
    () => (products.data ?? []).filter((p: any) => p.stock <= 5 && p.is_active),
    [products.data],
  );
  ...
  return (
    <main className="mx-auto max-w-6xl p-4 space-y-4">
      ...
      <form
        className="bg-white border rounded p-3 grid md:grid-cols-3 gap-2"
      ...
      >
        ...
        />
        <button className="border rounded px-3 py-2 md:col-span-3">
          Create Product
        </button>
      </form>
          ...
          <div key={p.id} className="bg-white border rounded p-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => void toggleActive(p.id, false)}
                >
                  Soft Delete
                </button>
              </div>
            ...
            <div className="mt-2 flex gap-2 items-center">
              <input
                ...
                onChange={(e) => void uploadImage(p.id, e.target.files?.[0])}
              />
              <span className="text-xs">Images: {p.images?.length ?? 0}</span>
            </div>
          </div>
        ))}
      </div>
